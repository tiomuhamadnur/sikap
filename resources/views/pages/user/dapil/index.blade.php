...
@section('modals')
    ...
    <div class="modal modal-blur fade" id="addKabupatenModal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-fromleft" role="document">
            <div class="modal-content">
                <form action="{{ route('relasi-dapil.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                    <div class="modal-header">
                        <h5 class="modal-title">Add New</h5>
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            <input type="hidden" class="form-control" id="dapil_id_add" name="dapil_id"
                                placeholder="Input dapil_id" autocomplete="off" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label required" for="kabupaten_id">Kabupaten/Kota</label>
                            <select class="form-select" name="kabupaten_id" id="kabupaten_id" required>
                                ...
                                @foreach ($kabupaten as $item)
                                    <option value="{{ $item->id }}">{{ $item->type }} {{ $item->name }} ({{ $item->code }})</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
...
@section('javascript')
    <script>
        $(document).ready(function() {
            ...
            $('#addKabupatenModal').on('show.bs.modal', function(e) {
